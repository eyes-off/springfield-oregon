# Archival Document Repository
## Springfield, Oregon

This data is made available under the [Creative Commons Zero](https://creativecommons.org/public-domain/cc0/) license in order to ensure it remains in the public domain.

This repository is intended to provide a historical archive of public records, reports, and other data from the city of Springfield, Oregon. 

These files have been provided as part of the EyesOff effort to archive and make available public record documents detailing mass surveillance.

For more information see [EyesOffEugene.org](https://eyesoffeugene.org).

## Public Records Requests
# Active Requests
| Label                                 | Submitted | Status                                     | Fee                | Link   |
|---------------------------------------|-----------|--------------------------------------------|--------------------|--------|
| Inventory of Surveillance Tech        | 8/17/2025 | Open - Fee Issued                          | $432.50 <br>Unpaid | [Link](https://github.com/eyes-off/springfield-oregon/tree/main/PublicRecords/2025_InventoryOfSurveillanceTech) |
| Oregon Retail Grant Reports - Updates | 11/3/2025 | Open                                       |                    | [Link](https://github.com/eyes-off/springfield-oregon/tree/main/PublicRecords/ORT_Grant_Report_Updates%5B2511-11%5D)

# Inactive Requests
| Label                                      | Submitted | Status                 | Fee                 | Link   |
|--------------------------------------------|-----------|------------------------|---------------------|--------|
| Contracts, Communications, Grant Info      | 7/5/2025  | Closed - Fulfilled     | $900 <br>Paid       | [Link](https://github.com/eyes-off/springfield-oregon/tree/main/PublicRecords/2023-2025_ORT_Grant) |

## Audit Data
* [Local Search Audit Logs](https://github.com/eyes-off/springfield-oregon/tree/main/FlockAuditLogs)  (i.e., Searches preformed by EPD officers)

## Audit Reports
| Publish Date | Name                                                           | Link |
|--------------|----------------------------------------------------------------|------|
| 8/25/2025    | Springfield Police ORT Grant       | [Link](https://github.com/eyes-off/springfield-oregon/blob/main/Reports/Report_%20Springfield%20Police%20ORT%20Grant%20-%202025-08-25.pdf) |
